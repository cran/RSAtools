#' @title Plots the response surface of a polynomial model of first, second, or third degree
#'
#' @description
...
#'
#' @name plotting.rsa
#' @param x Either an RSA object (returned by the \code{RSAmodel} function), or the coefficient for the X predictor
...
#' @return A plot of the response surface 
#'
#' @references
...
plotting.rsaCOEF <- function(x=0, y=0, x2=0, y2=0, xy=0, w=0, wx=0, wy=0, x3=0, xy2=0, x2y=0, y3=0, b0=0, 
	...
	points = list(data=NULL,n_random=NULL, show=NA, value="raw", jitter=0, color="black", cex=.5, out.mark=FALSE),
	...
	# if (class(x) == "RSA") {
		# stop("If you want to plot an RSA object, please use plotting.rsa(...); RSAplot_coef should be only used when you directly provide the regression coefficients.")
	# }
 ...
 # fit[[1]] <- c(fit[[1]],list("user"==NA))	
 # fit[[1]][["user"]] <- 	fit[[1]][[user_model]]
 # }
  is.cubicmodel <- model %in%  c("cubic","CA","RRCA","CL","RRCL","CUBIC","FM16_CUBICX","FM17_CUBICY", "FM18_LEVDEPQUADX","FM19_LEVDEPQUADY","FM20_ASYMCONG", "FM21_ASYMINCONG","FM22_LEVDEPCONG","FM23_LEVDEPINCONG","FM24_PARALLELASYM", "FM25_NONPARALLELASYM","FM26_PARALLELASYMWEAK","FM27_PARALLELASYMSTRONG","FM28_NONPARALLELASYMWEAK","FM29_NONPARALLELASYMSTRONG","FM30_ASYMCONGROTY","FM31_ASYMCONGROTX","FM32_ASYMINCONGROTY","FM33_ASYMINCONGROTX","FM34_LEVDEPCONGROTY","FM35_LEVDEPCONGROTX","FM36_LEVDEPINCONGROTY","FM37_LEVDEPINCONGROTX", model)
  ...
  # }
	...
	type <- match.arg(type, c("interactive", "3d", "contour","2d"))
	...
	if (is.null(axesStyles[["r1_LOC"]])) axesStyles[["r1_LOC"]] <- list(lty="solid", lwd=2, col=ifelse(bw==TRUE, "black", "green"))	
	if (is.null(axesStyles[["r2_LOC"]])) axesStyles[["r2_LOC"]] <- list(lty="solid", lwd=2, col=ifelse(bw==TRUE, "black", "green"))	
	if (is.null(axesStyles[["a1_LOC"]])) axesStyles[["a1_LOC"]] <- list(lty="twodash", lwd=2, col=ifelse(bw==TRUE, "black", "green"))	
	if (is.null(axesStyles[["a2_LOC"]])) axesStyles[["a2_LOC"]] <- list(lty="twodash", lwd=2, col=ifelse(bw==TRUE, "black", "green"))	
	if (is.null(axesStyles[["r1_LOIC"]])) axesStyles[["r1_LOIC"]] <- list(lty="solid", lwd=2, col=ifelse(bw==TRUE, "black", "red"))	
	if (is.null(axesStyles[["r2_LOIC"]])) axesStyles[["r2_LOIC"]] <- list(lty="solid", lwd=2, col=ifelse(bw==TRUE, "black", "red"))		
	if (is.null(axesStyles[["a1_LOIC"]])) axesStyles[["a1_LOIC"]] <- list(lty="twodash", lwd=2, col=ifelse(bw==TRUE, "black", "red"))	
	if (is.null(axesStyles[["a2_LOIC"]])) axesStyles[["a2_LOIC"]] <- list(lty="twodash", lwd=2, col=ifelse(bw==TRUE, "black", "red"))		
	...
	if (demo == FALSE) {
			...
			if (is.null(xlim)) {xlim <- c(-3.1, 3.1)}
			if (is.null(ylim)) {ylim <- c(-3.1, 3.1)}
	}
...
reversal_loic$Y_value <- as.numeric(reversal_loic$Y_value)
			...
			if ("r1_LOIC" %in% axes) {
				if(is.numeric(reversal_loic[c("r1_LOIC"),c("X_value")]) & !is.na(reversal_loic[c("r1_LOIC"),c("X_value")])){
			  p1 <- p1 + geom_abline(aes(intercept=-reversal_loic[c("r1_LOIC"),c("X_value")], slope=1), color="red")
				}			
			}
			if ("r2_LOIC" %in% axes) {
				if(is.numeric(reversal_loic[c("r2_LOIC"),c("X_value")]) & !is.na(reversal_loic[c("r2_LOIC"),c("X_value")])){
			  p1 <- p1 + geom_abline(aes(intercept=-reversal_loic[c("r2_LOIC"),c("X_value")], slope=1), color="red")
				}			
			}
			...
			if ("a1_LOIC" %in% axes) {
				if(is.numeric(acceleration_loic[c("a1_LOIC"),c("X_value")]) & !is.na(acceleration_loic[c("a1_LOIC"),c("X_value")])){
			  p1 <- p1 + geom_abline(aes(intercept=-acceleration_loic[c("a1_LOIC"),c("X_value")], slope=1), color="red",lty="twodash")
				}			
			}
			if ("a2_LOIC" %in% axes) {
				if(is.numeric(acceleration_loic[c("a2_LOIC"),c("X_value")]) & !is.na(acceleration_loic[c("a2_LOIC"),c("X_value")])){
			  p1 <- p1 + geom_abline(aes(intercept=-acceleration_loic[c("a2_LOIC"),c("X_value")], slope=1), color="red",lty="twodash")
				}			
			}
...
plot(niceplot[[2]])	
			...
			if (points$show == TRUE) {
								...
								if(!is.null(points$n_random)) {
# set.seed(123)
n_index <- sample(length(xpoints), points$n_random)
xpoints <- xpoints[n_index] 
ypoints <- ypoints[n_index] 
}
...
plotting.rsa <- function(x, ...) {
	...
	do.call(plotting.rsaCOEF, as.list(extras), envir = parent.frame())
}
